...
export function RegionalGuidePage({ locale, eyebrow, title, dek, checked, sections, sources, homeLabel, releaseLabel, consoleLabel, checkedLabel, sourcesLabel, footer }: RegionalGuideProps) {
  ...
  return <main className="doc-shell" lang={locale}><nav className="doc-nav"><a className="brand" href={base}>DAWNWALKER <span>GUIDE</span></a><div><a href={`${base}/release-date`}>{releaseLabel}</a><a href={`${base}/console-performance`}>{consoleLabel}</a></div></nav><article className="doc"><p className="eyebrow">{eyebrow}</p><h1>{title}</h1><p className="doc-dek">{dek}</p><p className="checked">{checkedLabel}: {checked}</p>{sections.map((section, index) => <Fragment key={section.title}><section><h2>{section.title}</h2>{section.body}</section>{index === middleIndex && <MiddleAd />}</Fragment>)}<section><h2>{sourcesLabel}</h2><ul className="sources">{sources.map((source) => <li key={source.href}><a href={source.href} target="_blank" rel="noreferrer">{source.label} ↗</a></li>)}</ul></section></article><footer><a className="brand" href={base}>Dawnwalker <span>Guide</span></a><p>{footer}</p></footer><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} /></main>;
}
