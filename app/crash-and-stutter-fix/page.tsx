...
export default function CrashAndStutterFixPage() {
  return <GuidePage
    ...
    sources={[
      ...
      { label: "Steam Community — player reports of frequent crashes after updates (September 14–15 sample)", href: "https://steamcommunity.com/app/3751260/discussions/0/588436698284996178/" },
    ]}
    sections={[
      { title: "Current community report: repeatable crashes after updates", body: <><div className="risk-callout"><span>COMMUNITY SAMPLE · NOT A CONFIRMED ROOT CAUSE</span><p><b>A fresh Steam thread has several players reporting new repeatable crashes after recent updates</b>, including in cutscenes, one boss fight and when closing inventory. The thread is useful for pattern tracking, but it does not establish one universal cause or an official workaround.</p></div><div className="fact-grid"><p><b>First safe test</b>Install the latest game update, verify files and retest the same scene.</p><p><b>Remove unsupported variables</b>Temporarily disable mods, injectors and overlays before retrying.</p><p><b>Make a useful report</b>Record the game version, platform, GPU driver, save state and exact trigger.</p><p><b>Do not jump to firmware</b>A random crash is not evidence that the shader-compilation BIOS guidance applies.</p></div></> },
      ...
      { title: "Performance issue versus an underpowered setup", body: <p>Low FPS is not automatically a launch bug. Compare your CPU, GPU, RAM, VRAM, resolution, upscaler mode and preset against the <Link href="/can-i-run">official PC requirements</Link>. A useful report includes frame-time behavior and location, rather than only a GPU name or a single average FPS number.</p> },
    ]}
  />;
}
