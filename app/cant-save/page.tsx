...
export default function CantSavePage() {
  return <GuidePage
    ...
    sections={[
      { title: "Two-minute save-lock check", body: <div className="fact-grid"><p><b>In combat, a cutscene or a visible scripted sequence?</b>Let that normal sequence finish, then check the save menu once.</p><p><b>Locked after a normal transition?</b>Record the quest, location, in-game day, version and last autosave before doing anything else.</p><p><b>Still locked after Hotfix 1.0.4?</b>Stop the long session, preserve the evidence and report the reproducible state. Do not overwrite the usable run.</p><p><b>Do not confuse patch scope</b>Hotfix 1.0.5 improves shader compilation; it is not an additional save-lock fix.</p></div> },
      { title: "First: tell a temporary save lock from a persistent bug", body: <div className="fact-grid"><p><b>Temporary lock</b>Some games disable saving during an active sequence, combat or a transition. Wait for the sequence to finish and check for the exact in-game notice.</p><p><b>Persistent lock</b>If the same message remains after a normal transition, note the active quest, location, in-game day and last autosave before taking another action.</p><p><b>One official fix, not a universal claim</b>Hotfix 1.0.4 lists a fix for an issue that blocked players from saving. It does not say that every locked-save report shares that cause.</p><p><b>Do not guess from the label</b>“Can&apos;t save,” a missing autosave and a story consequence that cannot be reversed are different problems.</p></div> },
