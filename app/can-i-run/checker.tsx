...
export default function CompatibilityChecker({ locale = "en" }: { locale?: Locale }) {
  ...
  const checks: Check[] = [
    ...
    { label: text.storage, value: storage ? `${storage} GB ${ssd === "yes" ? "SSD" : ssd === "no" ? text.hdd : ""}` : text.notProvided, minimum: locale === "pl" ? "60 GB na SSD" : "60 GB on SSD", recommended: locale === "pl" ? "60 GB na SSD" : "60 GB on SSD", state: !storage || ssd === "unknown" ? "unknown" : Number(storage) < 60 || ssd === "no" ? "fail" : "pass" },
  ...
  return <div className="checker"><div className="checker-intro"><div><p className="eyebrow">{text.localCheck}</p><h2>{text.heading}</h2><p>{text.intro}</p></div><button className="detect-button" onClick={useLocalDetection}>{text.detect}</button></div>{detected && <p className="detected">{detected}</p>}
    <div className="checker-fields"><label>{text.cpu}<input value={cpu} onChange={(event) => setCpu(event.target.value)} placeholder={text.exampleCpu} /></label><label>{text.gpu}<input value={gpu} onChange={(event) => setGpu(event.target.value)} placeholder={text.exampleGpu} /></label><label>{text.installedRam}<input type="number" min="1" value={ram} onChange={(event) => setRam(event.target.value)} placeholder="16" /></label><label>{text.gpu} VRAM (GB)<input type="number" min="1" value={vram} onChange={(event) => setVram(event.target.value)} placeholder="8" /></label><label>{text.freeStorage}<input type="number" min="1" value={storage} onChange={(event) => setStorage(event.target.value)} placeholder="60" /></label><label>{text.storageType}<select value={ssd} onChange={(event) => setSsd(event.target.value)}><option value="unknown">{text.unknown}</option><option value="yes">{text.ssd}</option><option value="no">{text.hdd}</option></select></label><label>{text.operatingSystem}<select value={os} onChange={(event) => setOs(event.target.value)}><option value="unknown">{text.unknown}</option><option value="Windows 10">Windows 10</option><option value="Windows 11">Windows 11</option><option value="Other">{locale === "pl" ? "Inny" : "Other"}</option></select></label></div><button className="primary compare-button" onClick={() => setChecked(true)}>{text.compare}</button>
